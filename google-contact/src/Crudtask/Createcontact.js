...
export default function Createcontact() {
    ...
    if(birthday==='')
    {
        let error = document.getElementById('birthday_error');
        error.innerHTML = "enter the birthday";
    }
    else{
        document.getElementById('birthday_error').innerHTML = '';
    }

    if(fname!=='' && lname!=='' && company!==''&& job!==''&& email!=='' && phone!=='' && birthday!==''){  
        axios.post('http://localhost:7000/write',{firstname:fname,lastname:lname,company:company,job:job,email:email,phone:phone});
    ...
    <div className='container'>
       <form onSubmit={handleSubmit} id="form">                
            <table className="table" id="createtable">
                    ...
                    <tr>
                        <td><label><FaBirthdayCake id="icon"/></label>
                        <input type='date' placeholder='Birthday' value={birthday} onChange={(e)=>setbirthday(e.target.value)} onKeyUp={removewarning6}/>
                        <span id="birthday_error" className="error text-danger"></span></td>
                    </tr>
                    <tr>
